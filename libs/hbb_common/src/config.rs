...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("SeeDesktop".to_owned());
...
pub const LINK_DOCS_HOME: &str = "https://seedesktop.com/docs/en/";
pub const LINK_DOCS_X11_REQUIRED: &str = "https://seedesktop.com/docs/en/manual/linux/#x11-required";
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["api.seedesktop.com"];
pub const RS_PUB_KEY: &str = "KafXrl4RSnR62ccbm15Jk8kd7sBSYaW9JxSXbGw8224=";
...
impl Config {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn app_root_by_name(app_name: &str) -> Option<PathBuf> {
        #[cfg(not(target_os = "macos"))]
        let org = "".to_owned();
        #[cfg(target_os = "macos")]
        let org = ORG.read().unwrap().clone();
        directories_next::ProjectDirs::from("", &org, app_name)
            .map(|project| patch(project.config_dir().to_path_buf()))
    }

    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn migrate_legacy_rustdesk_data_if_needed() {
        // Keep existing users' local data when migrating branded app directory.
        if APP_NAME.read().unwrap().as_str() != "SeeDesktop" {
            return;
        }
        let Some(new_root) = Self::app_root_by_name("SeeDesktop") else {
            return;
        };
        let Some(old_root) = Self::app_root_by_name("RustDesk") else {
            return;
        };
        if !old_root.exists() {
            return;
        ...
        for suffix in config_suffixes {
            let old_name = format!("RustDesk{suffix}.toml");
            let new_name = format!("SeeDesktop{suffix}.toml");
            let old_file = old_root.join(old_name);
            let new_file = new_root.join(new_name);
            if old_file.exists() && !new_file.exists() {
                let _ = fs::copy(old_file, new_file);
            }
        }

        let old_peers_dir = old_root.join(PEERS);
        let new_peers_dir = new_root.join(PEERS);
        if old_peers_dir.exists() {
            let _ = fs::create_dir_all(&new_peers_dir);
            if let Ok(entries) = old_peers_dir.read_dir() {
                for entry in entries.flatten() {
                    ...
                    let new_path = new_peers_dir.join(name);
                    if !new_path.exists() {
    ...
    fn load() -> Config {
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        Self::migrate_legacy_rustdesk_data_if_needed();
    ...
    pub fn get_rendezvous_server() -> String {
        let mut rendezvous_server = "api.seedesktop.com".to_owned();
    ...
    pub fn get_rendezvous_servers() -> Vec<String> {
        vec!["api.seedesktop.com".to_owned()]
    }
...
impl PeerConfig {
    ...
    fn default_options() -> HashMap<String, String> {
        ...
        mp.insert(keys::OPTION_ACCESS_MODE.to_owned(), "full".to_owned());
        mp.insert(keys::OPTION_ENABLE_KEYBOARD.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_CLIPBOARD.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_FILE_TRANSFER.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_CAMERA.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_TERMINAL.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_AUDIO.to_owned(), "Y".to_owned());
        mp.insert(keys::OPTION_ENABLE_TUNNEL.to_owned(), "Y".to_owned());
        mp.insert(
            keys::OPTION_ENABLE_REMOTE_RESTART.to_owned(),
            "Y".to_owned(),
        );
        mp.insert(
            keys::OPTION_ENABLE_RECORD_SESSION.to_owned(),
            "Y".to_owned(),
        );
        mp.insert(keys::OPTION_ENABLE_BLOCK_INPUT.to_owned(), "Y".to_owned());
        mp.insert(
